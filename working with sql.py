...
def db_show_table_books():
    ...
    result = cursor.fetchall()
    for i in range(0,len(result)):
        print("№", i+1, "ID:", result[i][0], ". Название книги:", result[i][1], "Автор:", result[i][2], "Год издания:", result[i][3])
...
def db_set_default():
    try:
        db_create_tables_authors()
        db_create_table_books()
        print("База данных упешно заполнена")
    except sqlite3.IntegrityError:
        print("База данных уже заполнена дефолтными значениями")
...
def db_add_book():
    ...
    if flag:
        try:
            cursor.execute("INSERT INTO books(id,title,author,published) VALUES(?,?,?,?)", (id, title, author, published))
...
def show_menu():
    ...
    print("2 - заполнить таблицу дефолтными значениями")
...
with sqlite3.connect("InfoBook.db") as db:
    cursor = db.cursor()
...
cursor.execute("CREATE TABLE IF NOT EXISTS authors(name text PRIMARY KEY,place_of_birth text NOT NULL);")
